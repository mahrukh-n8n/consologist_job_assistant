...
must_haves:
  truths:
    ...
    - "The CSV file has exactly 44 column headers matching the reference project field names (case-sensitive): category, subcategory, postedAt, publishTime, createTime, lastActivity, lastOnlineTime, currencyCode, hourlyMin, hourlyMax, projectBudget, weeklyRetainerBudget, engagementDuration, engagementWeeks, hourlyEngagementType, Project Payment Type, skills, contractorTier, totalApplicants, totalInvitedToInterview, totalHired, unansweredInvites, invitationsSent, numberOfPositionsToHire, hireRate, clientCountry, clientCity, feedbackScore, feedbackCount, totalCharges, totalJobsWithHires, openedJobs, isPaymentVerified, clientIndustry, clientCompanySize, qualifications_regions, qualifications_worldRegion, qualifications_country, minJobSuccessScore, englishLevel, Title, URL, Description, Job ID"
...
NOTE: Field names are the reference project schema (case-sensitive). The service worker
transforms scraper output via job-transformer.js before storing lastScrapedJobs, so
CsvExporter receives pre-transformed objects with these exact field names.
...
At the top of the file, add imports alongside existing imports:
```js
import { CsvExporter } from '../utils/csv-exporter.js';
import { transformJob } from '../utils/job-transformer.js'; // already imported from PUSH_JOBS
```

NOTE: transformJob is already imported (from PUSH_JOBS plan). No duplicate import needed.
